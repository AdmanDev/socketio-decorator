...
describe("> CurrentSocket Decorator", () => {
	...
	class ControllerTest {
		...
		public onSimpleTest (someParam: ServerSocket, @CurrentSocket() socket: ServerSocket) {
			simpleTestFn(someParam.id, socket.id)
	...
	describe("> Functional tests", () => {
		it("should inject socket into the method parameter", (done) => {
			clientSocket.on("simple-test-resp", () => {
				expect(simpleTestFn).toHaveBeenCalledWith(clientSocket.id, clientSocket.id)
				done()
			})
