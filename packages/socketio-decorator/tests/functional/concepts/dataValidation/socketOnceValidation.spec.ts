...
import { CurrentSocket, Data, IErrorMiddleware, SiodIncomingDataError, SocketOnce } from "../../../../src"
...
describe("> @SocketOnce Data Validation", () => {
	let io: Server
	...
	class SocketOnceValidationController {
		@SocketOnce("with-validation")
		public onWithValidation (@CurrentSocket() socket: ServerSocket, @Data() data: MessageData) {
			controllerFnSpy(data, socket.id)
		}

		@SocketOnce("no-validation", { disableDataValidation: true })
	...
	beforeAll((done) => {
		io = createServer(
			{
				controllers: [SocketOnceValidationController],
